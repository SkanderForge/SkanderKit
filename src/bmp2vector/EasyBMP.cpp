...
int BMP::TellHeight(void) const { return Height; }

// int BMP::TellWidth( void ) const
int BMP::TellWidth(void) const { return Width; }
...
bool BMP::SetBitDepth(int NewDepth) {
    ...
    if (BitDepth == 1 || BitDepth == 4 || BitDepth == 8) { Colors = new RGBApixel[NumberOfColors]; }
    else { Colors = NULL; }
...
bool BMP::WriteToFile(const char *FileName) {
    ...
    if (BitDepth == 1 || BitDepth == 4 || BitDepth == 8) { dPaletteSize = IntPow(2, BitDepth) * 4.0; }
...
bool SafeStringRead(char *buffer, int size, int number, std::istringstream &ss) {
    if (ss.eof()) {
        return false;
    }

    ss.read(buffer, size * number);
    if (ss.gcount() < size * number) {
        return false;
    }
...
bool BMP::ReadFromString(const std::string &bmpContent) {
    using namespace std;
    istringstream ss(bmpContent, ios_base::in | ios_base::binary);

    size_t contentIndex = 0; // use this to track where we are in the string
    if (bmpContent.empty()) {
        SetBitDepth(1);
        SetSize(1, 1);
        return false;
    ...
    NotCorrupted &= SafeStringRead((char *) &(bmfh.bfSize), sizeof(ebmpDWORD), 1, ss);
    NotCorrupted &= SafeStringRead((char *) &(bmfh.bfReserved1), sizeof(ebmpWORD), 1, ss);
    NotCorrupted &= SafeStringRead((char *) &(bmfh.bfReserved2), sizeof(ebmpWORD), 1, ss);
    NotCorrupted &= SafeStringRead((char *) &(bmfh.bfOffBits), sizeof(ebmpDWORD), 1, ss);
    if (IsBigEndian()) { bmfh.SwitchEndianess(); }
// read the info header
    BMIH bmih;
    NotCorrupted &= SafeStringRead((char *) &(bmih.biSize), sizeof(ebmpDWORD), 1, ss);
    NotCorrupted &= SafeStringRead((char *) &(bmih.biWidth), sizeof(ebmpDWORD), 1, ss);
    NotCorrupted &= SafeStringRead((char *) &(bmih.biHeight), sizeof(ebmpDWORD), 1, ss);
    NotCorrupted &= SafeStringRead((char *) &(bmih.biPlanes), sizeof(ebmpWORD), 1, ss);
    NotCorrupted &= SafeStringRead((char *) &(bmih.biBitCount), sizeof(ebmpWORD), 1, ss);
    NotCorrupted &= SafeStringRead((char *) &(bmih.biCompression), sizeof(ebmpDWORD), 1, ss);
    NotCorrupted &= SafeStringRead((char *) &(bmih.biSizeImage), sizeof(ebmpDWORD), 1, ss);
    NotCorrupted &= SafeStringRead((char *) &(bmih.biXPelsPerMeter), sizeof(ebmpDWORD), 1, ss);
    NotCorrupted &= SafeStringRead((char *) &(bmih.biYPelsPerMeter), sizeof(ebmpDWORD), 1, ss);
    NotCorrupted &= SafeStringRead((char *) &(bmih.biClrUsed), sizeof(ebmpDWORD), 1, ss);
    NotCorrupted &= SafeStringRead((char *) &(bmih.biClrImportant), sizeof(ebmpDWORD), 1, ss);
    if (IsBigEndian()) { bmih.SwitchEndianess(); }
    if (!NotCorrupted) {
        SetSize(1, 1);
        SetBitDepth(1);
           ...
           && TempBitDepth != 24 && TempBitDepth != 32 )
    {
        if( EasyBMPwarnings )
        {
            cout << "EasyBMP Error: " << "" << " has unrecognized bit depth." << endl;
        }
        SetSize(1,1);
        SetBitDepth(1);
        return false;
    ...
    if (BitDepth < 16) {
        ...
        if (NumberOfColorsToRead < TellNumberOfColors()) {
            if (EasyBMPwarnings) {
                cout << "EasyBMP Warning: file " << "" << " has an underspecified" << endl
                     << "                 color table. The table will be padded with extra" << endl
                     << "                 white (255,255,255,0) entries." << endl;
            }
        }
        ...
        int n;
        for (n = 0; n < NumberOfColorsToRead; n++) {
            SafeStringRead((char *) &(Colors[n]), 4, 1, ss);
        }
        for (n = NumberOfColorsToRead; n < TellNumberOfColors(); n++) {
            RGBApixel WHITE;
            WHITE.Red = 255;
    ...
    if (BytesToSkip > 0 && BitDepth != 16) {
        ebmpBYTE *TempSkipBYTE;
        TempSkipBYTE = new ebmpBYTE[BytesToSkip];
        SafeStringRead((char *) TempSkipBYTE, BytesToSkip, 1, ss);
        delete[] TempSkipBYTE;
    }
    int i, j;
    if (BitDepth != 16) {
    ...
    if (BitDepth == 16) {
        ...
        if (bmih.biCompression != 0) {
            ...
            SafeStringRead((char *) &RedMask, 2, 1, ss);
            if (IsBigEndian()) { RedMask = FlipWORD(RedMask); }
            SafeStringRead((char *) &TempMaskWORD, 2, 1, ss);

            SafeStringRead((char *) &GreenMask, 2, 1, ss);
            if (IsBigEndian()) { GreenMask = FlipWORD(GreenMask); }
            SafeStringRead((char *) &TempMaskWORD, 2, 1, ss);

            SafeStringRead((char *) &BlueMask, 2, 1, ss);
            if (IsBigEndian()) { BlueMask = FlipWORD(BlueMask); }
            SafeStringRead((char *) &TempMaskWORD, 2, 1, ss);
        }
        if (BytesToSkip > 0) {
...
// read the actual pixels
        ...
        for (j = Height - 1; j >= 0; j--) {
            ...
            while (ReadNumber < DataBytes) {
                ebmpWORD TempWORD;
                SafeStringRead((char *) &TempWORD, 2, 1, ss);
                if (IsBigEndian()) { TempWORD = FlipWORD(TempWORD); }
                ReadNumber += 2;
                std::cout << "G" << std::endl;
                ebmpWORD Red = RedMask & TempWORD;
            ...
            while (ReadNumber < PaddingBytes) {
                ebmpBYTE TempBYTE;
                SafeStringRead((char *) &TempBYTE, 1, 1, ss);
                ReadNumber++;
            }
        }
    }
